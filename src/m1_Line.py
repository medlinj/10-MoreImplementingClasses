...
class Line(object):
    ...
    def slope(self):
        ...
        if self.end.x == self.start.x:
            return math.inf
        calc = (self.end.y - self.start.y) / (self.end.x - self.start.x)
        return calc
    ...
    def line_minus(self, other_line):
        ...
        diff_start_x = self.start.x - other_line.start.x
        diff_start_y = self.start.y - other_line.start.y
        diff_end_x = self.end.x - other_line.end.x
        diff_end_y = self.end.y - other_line.end.y
        return Line(Point(diff_start_x, diff_start_y), Point(diff_end_x, diff_end_y))
